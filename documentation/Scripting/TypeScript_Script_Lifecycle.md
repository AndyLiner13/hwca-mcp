...
6. `dispose()`: `dispose()` is called when a TypeScript component instance is destroyed, when the world is
...
In the following example, the script registers a listener to a local event
